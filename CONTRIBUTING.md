...
Profile writes use a per-profile-root advisory lock file before creating, removing, renaming, replacing, or extending stored profile directories. Lock acquisition retries briefly and reports a concrete stuck-writer diagnostic after a bounded wait instead of hanging forever. Keep new profile-store mutations inside `ProfileStore` so cross-process coordination stays at one filesystem boundary instead of being duplicated in runtime scheduling or voice-operation callers.
...
Removed generation-context keys such as `input_text_context`, `text_format`, and `nested_source_format`, plus removed request-context keys such as `app`, `agent`, and `project`, are intentionally rejected. That keeps stale callers from silently dropping path context, source-format intent, or request metadata after the `TextForSpeech` request-context simplification.
...
{"id":"req-default","op":"get_default_voice_profile"}
{"id":"req-default-set","op":"set_default_voice_profile","voice_profile":"swift-anchor"}
