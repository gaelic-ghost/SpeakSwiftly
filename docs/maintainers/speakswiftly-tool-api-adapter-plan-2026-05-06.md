...
| `create_system_voice_profile_from_description` | `.createProfile(..., author: .system)` | Tool-only system profile creation | Implemented through `SpeakSwiftly.Tool.createBuiltInVoiceProfile(...)`; intentionally not exposed on `runtime.voices` |
