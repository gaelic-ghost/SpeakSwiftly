...
- [ ] Convert bucket 72 and bucket 88 if parity and package size remain manageable.
- [ ] Scope decoder activation quantization away from integer audio-code inputs, or split code lookup into a separate graph, before deciding whether the speech-tokenizer decoder can or cannot use W8A8.
...
- The first Core ML quantization preflight confirms Core ML Tools 9 exposes both int8 weight quantization and experimental activation quantization APIs, but the current decoder package is fixed to `[1, 8, 16]`. That package can only support a synthetic W8A8 smoke test; representative W8A8 calibration is blocked until bucketed decoder packages match the real-speech calibration shapes.
- A local synthetic quantization smoke showed weight-only int8 can shrink the decoder package from about 436 MB to about 110 MB, while naive global W8A8 activation quantization fails by inserting a quantize op on an `int32` audio-code path. The next W8A8 pass should scope quantization to float decoder work rather than treating that as a final ANE blocker.
...
- Bucket 40 converted successfully through the existing static-mask export path. CPU-only Core ML prediction produced `[1, 76800]` samples with max absolute difference below `0.0003` versus the PyTorch wrapper, and the local `.mlpackage` remained about 436 MB.
