...
- The first real Core AI probes now have matched Qwen3-TTS outputs for the code predictor and main-talker frozen-cache decode path. The code predictor converts to Core AI IR, and the main talker converts in float32 with exact exported-program parity, but BF16 main-talker Core AI conversion currently fails on a `dtype f32 vs bf16` lowering issue. Treat Core AI as promising but still gated on BF16 lowering, mutable cache state, runtime profiling, and end-to-end quality.
