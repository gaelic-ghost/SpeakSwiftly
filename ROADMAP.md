...
- [x] Probe Core AI code-predictor weight-only W8 compression through `coreai-opt` and `coreai-torch`; eager mode converts to Core AI IR but the compressed logits drift too far to compile/profile for ANE yet.
...
- The first `coreai-opt` code-predictor W8 probe found two important boundaries: graph-mode W8 needs explicit per-channel axis configuration and then hits a functorch/vmap prepared-graph failure; eager-mode W8 with `PerChannelGranularity(axis=0)` reaches `coreai-torch` conversion, but the compressed logits collapse enough to show max absolute drift `12.8125`. Do not compile/profile that W8 graph for ANE until compression parity is fixed.
