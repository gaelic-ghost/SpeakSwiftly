...
- The decoder closeout baseline now has a matching fp16 two-bucket resident catalog. In this probe shape, fp16 and W8A8 `linear` + `matmul` have similar pass-two hot timing, while W8A8 halves package size but pays larger first-use warmups. Keep W8A8 `linear` + `matmul` as the only current decoder W8A8 candidate, keep convolution and upsampling activations fp16, require explicit resident bucket warmup, and keep decoder quality pending until manual listening is complete.
