...
- The Swift probe now also supports a resident bucket catalog with repeatable `--bucket-model` entries. The first catalog run loaded bucket 72 and bucket 88 once in the same process, routed prompt-001 to bucket 72 at about `84.3 ms`, routed prompt-002 to bucket 88 at about `109.6 ms`, and kept both paths in the same private probe instead of introducing a public backend case.
