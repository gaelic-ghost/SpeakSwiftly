...
- [ ] Run a Core AI runtime-feasibility pass for the real Qwen3-TTS talker/code-predictor slice, capturing export, load, execution, output drift, failure modes, and any runtime API limits in one checked-in evidence report.
- [ ] Decide whether Core AI compressed or palettized Qwen3-TTS code-predictor graphs have a credible ANE route, using `coreai-build --preferred-compute neural-engine`, `coreai-build inspect --json`, Instruments evidence, and parity/drift gates instead of assuming ANE benefit from W8/W8A8 configuration alone.
- [ ] Refresh the runtime-choice matrix after the Core AI runtime pass so hand-rolled Core ML probes, Core AI through `coreai-torch`, ExecuTorch Core ML, ExecuTorch MLX, and direct MLX Swift are compared against the same conversion, execution, quality, ANE-dispatch, and integration criteria.
...
- The next Core AI slice should answer runtime feasibility directly: can a real talker/code-predictor slice be exported, loaded, executed, and measured with acceptable output drift? If yes, deepen Core AI. If no, keep Core AI as conversion/compression evidence and pivot runtime work toward ExecuTorch MLX, ExecuTorch Core ML comparison, or direct MLX Swift.
