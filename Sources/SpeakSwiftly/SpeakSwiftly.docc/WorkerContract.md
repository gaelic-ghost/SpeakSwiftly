...
For generation requests, the current worker keys are `voice_profile`, `text_profile`, `source_format`, and `request_context`. Omit `voice_profile` to use the runtime default voice profile, which falls back to `swift-signal`. `source_format` is only for whole-source input; mixed prose, Markdown, logs, HTML, CLI output, and agent text should omit source hints and let `TextForSpeech` detect structure internally. `request_context` uses the shared `TextForSpeech.RequestContext` shape, including `source`, `topic`, `attributes`, `cwd`, and `repo_root` context. Removed generation-context keys such as `input_text_context`, `text_format`, and `nested_source_format`, plus removed request-context keys such as `app`, `agent`, and `project`, are rejected with explicit invalid-request diagnostics. Qwen live playback can also opt into pre-model text chunking with `qwen_pre_model_text_chunking: true`; when omitted, Qwen live playback remains single-pass. Qwen resident model selection is part of the startup backend value: use `qwen3_smol`, `qwen3_smol_6bit`, `qwen3_smol_8bit`, or `qwen3_smol_bf16` for the Qwen 0.6B resident model family, and `qwen3_big`, `qwen3_big_6bit`, `qwen3_big_8bit`, or `qwen3_big_bf16` for the Qwen 1.7B resident model family. Prepared Qwen conditioning is stored per resident model repo so a profile can lazily accumulate conditioning for each selected Qwen model. Rerolling a profile regenerates every existing prepared Qwen conditioning artifact for that profile, plus the currently selected Qwen backend when applicable. Older generation-request aliases such as `profile_name` and `text_profile_id` are still accepted for compatibility.
...
`create_system_voice_profile_from_description` is a development-time resource authoring operation. Start `SpeakSwiftlyTool` with `--system-profile-resource-root PATH` before using it; the tool writes the generated system profile under `PATH/profiles/` so the directory can be bundled with the `SpeakSwiftly` target. In this resource-authoring mode, `SpeakSwiftlyTool` starts with resident playback models unloaded and prepared Qwen conditioning remains lazy. Runtime startup seeds bundled system profiles from the package resource bundle into the writable profile store. Without `--system-profile-resource-root`, system-profile creation is rejected instead of installing package-owned profiles into ordinary runtime state.
