...
enum SpeechTextNormalizer {
	...
	static func forensicFeatures(originalText: String, normalizedText: String)
		-> SpeechTextForensicFeatures
	{
		...
		return SpeechTextForensicFeatures(
			...
			markdownHeaderCount: originalText.split(
		...
		return sections.enumerated().map { index, section in
			...
			let durationMS =
				...
				: min(
					remainingDurationMS,
					max(
						Int((Double(totalDurationMS) * section.normalizedCharacterShare).rounded()),
						0)
				)
			let chunkCount =
				isLastSection
				...
				: min(
					remainingChunkCount,
					max(
						Int((Double(totalChunkCount) * section.normalizedCharacterShare).rounded()),
						0)
				)
...
extension SpeechTextNormalizer {
	...
	private static func aliasedPathPrefix(in text: String) -> (
		range: Range<String.Index>, spokenName: String
	)? {
		let aliases = [
