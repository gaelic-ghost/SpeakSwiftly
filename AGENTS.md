...
- Expose stored concern handles such as `generate`, `playback`, `voices`, `normalizer`, `jobs`, and `artifacts` from `SpeakSwiftly.Runtime` instead of growing one monolithic method namespace. The current `player` handle is scheduled for a breaking rename to `playback` under Milestone 28.
