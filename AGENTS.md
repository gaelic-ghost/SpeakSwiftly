...
- Treat the required GitHub Actions PR/main lane as the lightweight `validate` job. The heavier Xcode-backed package, iOS smoke, and tagged runtime publication workflow in `.github/workflows/swift.yml` is for manual dispatch and tagged releases, not every pull request.
